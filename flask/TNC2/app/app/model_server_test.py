...
BATCH_SIZE = 1 # Change only when Tensorflow GPU is used
    ...
    while True:
        ...
        if len(imageIDs) > 0:
            ...
            for i in range(len(imageIDs)):
                ...
                image = Image.open(imageIDs[i]).convert("RGB")
                ...
                plt.axis('off')
                plt.savefig(os.path.join("static/detected_img/", filename_extractor(imageIDs[i])), dpi=100, transparent=True, optimize=True, quality=90)
                ...
                progress = []
                percent = i/len(imageIDs) * 100.00
                progress.append(percent)
                msg = str(i) + "of" + str(len(imageIDs)) + "completed."
                progress.append(msg)
