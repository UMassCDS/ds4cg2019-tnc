...
def build(train_config, num_classes=1):
    # set cross entropy as a default
    if 'criterion' not in train_config:
        criterion_config = {'gamma': 0, 'alpha': None, 'num_classes': 1}
    else:
        criterion_config = train_config['criterion']
        if 'alpha' not in criterion_config:
            criterion_config['alpha'] = None

    if num_classes > 1:
        criterion = MultiClassFocalLoss(**criterion_config)
    else:
        criterion = BinaryClassFocalLoss(**criterion_config)

    log.infov(
        'Focal loss with gamma={}, alpha={} is built'.format(
            criterion_config['gamma'], criterion_config['alpha'])
    )
    return criterion
